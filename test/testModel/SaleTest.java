...
public class SaleTest {
    ...
    public void validaciones_constructor_y_setters() {
        ...
        assertThrowsIAE(() -> new Sale("", now, 1));
        assertThrowsIAE(() -> new Sale("V1", null, 1));

        // mesa inválida (solo 1..5 o 0 para llevar)
        assertThrowsIAE(() -> new Sale("V1", now, -1));
        assertThrowsIAE(() -> new Sale("V1", now, 6));

        // tax negativo
        assertThrowsIAE(() -> new Sale("V1", now, 1, -0.01));
    }

    @Test
    public void paraLlevar_funciona() {
        Sale v = new Sale("V1", LocalDateTime.now(), Sale.PARA_LLEVAR);
        assertTrue(v.esParaLlevar());
    }

    @Test
    public void agregarLinea_calculaSubtotalImpuestoTotal() {
        Sale v = new Sale("V1", LocalDateTime.now(), 1, 0.10); // 10%

        Product p1 = new Product("P1", "Prod1", "Cat", 1000.0, 10);
        Product p2 = new Product("P2", "Prod2", "Cat", 500.0, 10);
        ...
        List<Sale.LineaVenta> lineas = v.getLineas();
    ...
    public void agregarLinea_validaciones() {
        Sale v = new Sale("V1", LocalDateTime.now(), 1);
        ...
        try {
            v.agregarLinea(new Product("P1", "N", "C", 1.0, 1), 0);
    ...
    public void getLineas_esInmodificable() {
        Sale v = new Sale("V1", LocalDateTime.now(), 1);
        v.agregarLinea(new Product("P1", "N", "C", 100.0, 1), 1);

        List<Sale.LineaVenta> lineas = v.getLineas();
        try {
            lineas.add(new Sale.LineaVenta(new Product("P2", "N2", "C", 50.0, 1), 1, 50.0));
    ...
    public void toCsv_fromCsv_roundtrip() {
        ...
        Sale v = new Sale("V1", dt, 2, 0.13);

        v.agregarLinea(new Product("P1", "N1", "C", 1000.0, 10), 2);
        v.agregarLinea(new Product("P2", "N2", "C", 500.0, 10), 1);

        String csv = v.toCsv();
        Sale parsed = Sale.fromCsv(csv);
    ...
    public void fromCsv_invalido_lanza() {
        assertThrowsIAE(() -> Sale.fromCsv(null));
        assertThrowsIAE(() -> Sale.fromCsv(""));
        assertThrowsIAE(() -> Sale.fromCsv("a,b,c")); // incompleto
    }

    @Test
    public void LineaVenta_validaciones() {
        try {
            new Sale.LineaVenta(null, 1, 10.0);
            fail("Debe lanzar IllegalArgumentException por producto null.");
        } catch (IllegalArgumentException ex) { /* ok */ }

        try {
            new Sale.LineaVenta(new Product("P1", "N", "C", 1.0, 1), 0, 10.0);
            fail("Debe lanzar IllegalArgumentException por cantidad <= 0.");
        } catch (IllegalArgumentException ex) { /* ok */ }

        try {
            new Sale.LineaVenta(new Product("P1", "N", "C", 1.0, 1), 1, -1.0);
