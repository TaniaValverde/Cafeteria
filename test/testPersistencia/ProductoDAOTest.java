...
public class ProductoDAOTest {
    ...
    private ProductoDAO dao(Path archivo) {
        return new ProductoDAO(archivo.toString());
    }
    ...
    public void cargar_archivoNoExiste_devuelveListaVacia() throws Exception {
        ...
        List<Producto> productos = dao(archivo).cargar();
    ...
    public void guardar_listaVacia_creaArchivo_yCargarDevuelveVacia() throws Exception {
        Path archivo = tempDir.resolve("productos.txt");
        ProductoDAO d = dao(archivo);

        d.guardar(Collections.<Producto>emptyList());

        assertTrue("guardar() debe crear el archivo si no existe", Files.exists(archivo));

        List<Producto> cargados = d.cargar();
    ...
    public void guardar_yLuegoCargar_conservaDatos() throws Exception {
        ...
        lista.add(new Producto("P001", "Café", "Bebidas", 25.0, 10));
        lista.add(new Producto("P002", "Galleta", "Snacks", 12.5, 30));
        ...
        assertTrue(cargados.contains(new Producto("P001", "X", "X", 0, 0)));
        assertTrue(cargados.contains(new Producto("P002", "X", "X", 0, 0)));
    ...
    public void guardar_sobrescribeContenido_anterior() throws Exception {
        ...
        d.guardar(Arrays.asList(new Producto("P001", "Café", "Bebidas", 25.0, 10)));
        d.guardar(Arrays.asList(new Producto("P999", "Prueba", "Test", 1.0, 1)));

        List<Producto> cargados = d.cargar();
        assertEquals("Debe sobrescribir el contenido anterior", 1, cargados.size());
        assertTrue(cargados.contains(new Producto("P999", "X", "X", 0, 0)));
        assertFalse(cargados.contains(new Producto("P001", "X", "X", 0, 0)));
    }
    ...
    public void guardar_creaDirectorios_siNoExisten() throws Exception {
        ...
        d.guardar(Arrays.asList(new Producto("P010", "Té", "Bebidas", 10.0, 5)));
    ...
    public void guardar_enRutaQueEsDirectorio_debeLanzarIOException() throws Exception {
        ...
        ProductoDAO d = dao(carpeta); // OJO: ruta apunta a un DIRECTORIO
